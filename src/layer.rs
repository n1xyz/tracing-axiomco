...
mod tests {
    ...
    fn capture_event(emit: impl FnOnce()) -> serde_json::Value {
        let (tx, mut rx) = tokio::sync::mpsc::channel::<crate::Event>(1);
        let subscriber =
            tracing_subscriber::registry().with(crate::layer(tx.downgrade()));
    ...
    #[test]
    fn serde_fields_remain_structured_and_errors_fall_back_to_debug() {
        #[derive(Debug)]
        ...
        assert_eq!(event["data"]["failed"], "FailsToSerialize");
        assert_eq!(event["data"]["debug"], "[1, 2, 3]");
